...
### 실측 실패 양상 (2026-08-03 Phase 2 — 반드시 피하라)

Codex 첫 이관 시도에서 **실제로 나온** 두 가지 실패다. 둘 다 자기보고는 "completed"였고
당시 검증기는 통과시켰다. 지금은 검증기가 둘 다 FAIL 로 잡지만, 애초에 하지 마라.

1. **소스 복사본 생산 (치명적)** — 소스가 이미 한국어 번역본이면 "재진술 금지"를 "손대지 말라"로
   오독해 산문 문단 **100%를 한 글자도 바꾸지 않고** 옮긴 사례. 자수비 1.02x 라 분량 게이트를 그냥 통과했다.
   - **재진술 금지 ≠ 재작성 금지.** 금지되는 것은 *같은 내용을 두 번 쓰는 것*이지, 고쳐 쓰는 것이 아니다.
   - 한국어 소스에서 해설판의 부가가치는 **구조 재편(표·목록·소제목)·개념 보강·맥락 추가**다
     (§한국어 평이 소스 모드). 그 셋 중 아무것도 하지 않았다면 해설판을 만들지 않은 것이다.
   - 자가 점검: 출력 문단을 소스에서 그대로 찾을 수 있다면 그 문단은 **아직 작업하지 않은** 것이다.
2. **그림 대량 유실** — 소스 이미지 23개 중 21개(91%)를 버리고 분량도 0.52x 로 줄인 사례.
   - 이미지 참조 `![](images/...)` 보존은 **하드룰**이다 (§Formatting Preservation Checklist).
     본문에서 설명할 자리를 못 찾겠으면 해당 절에 그대로 두고 캡션을 달아라. 버리지 마라.
   - 분량이 소스의 절반으로 줄었다면 요약을 한 것이다. 이 스킬은 요약 스킬이 아니다 (Rule 0).
3. **수식·수치를 버리며 압축 (장문 기술 논문)** — 69K자 논문에서 인라인 수식을 584→229개(39%)로
   줄이고 본문 수치 153개 중 38개(25%)를 흘린 사례. 표 행도 16개가 사라졌다.
   자수비가 0.50x 라 "짧아졌다"로만 보였지 **무엇이 사라졌는지는 자수로 드러나지 않는다.**
   - 아래 §Evidence Inventory 를 반드시 수행하라. 이 실패는 절차 없이 감으로 쓰면 반드시 재발한다.

### Evidence Inventory (증거 대장) — 장문·수식·표가 있는 문서에서 필수

쓰기 **전에** 소스를 훑어 대장을 만들고, 쓰고 나서 대장을 대조한다. 순서를 지켜라.

1. **섹션별 대장 작성** — 소스의 각 `##`/`###` 절마다 그 절이 담고 있는 것을 적는다:
   - 수치(단위·조건 포함. `0.120 [0.075, 0.155]` 은 값 하나가 아니라 **값+구간 한 덩어리**다)
   - 표: 행 라벨과 그 행의 수치들. **행 단위로** 적는다.
   - 수식: 블록·인라인 각각 몇 개인지, 무엇을 정의하는지
   - 그림: 파일 경로와 캡션
2. **절마다 그 절을 쓴 직후 대조** — 대장의 항목이 방금 쓴 절에 다 들어갔는지 본다.
   문서를 다 쓰고 마지막에 몰아서 대조하지 마라. 그때는 이미 압축된 뒤다.
3. **표는 전 행을 옮긴다** — HTML 표를 마크다운 표로 바꾸는 것은 좋다(Rule 7 이 권장한다).
   하지만 **행을 빼는 것은 안 된다.** 행이 많아 부담되면 표를 그대로 싣고 해설을 덧붙여라.
4. **수식은 원형 보존 + 해설** (Rule 5). 해설을 붙였다고 원 수식을 지우지 마라.
   변수 정의만 남기고 식을 지우면 독자가 원문과 대조할 수 없다.
5. **누락을 찾으면 원래 문맥에 복원한다** — 그 값이 원래 있던 절에 되돌려 놓는다.

**금지 — 게이트 통과용 덤프:**
누락된 수치·표를 문서 끝의 용어집이나 "수치 인덱스", "부록: 원문 표" 같은
**해설 없는 목록에 몰아넣지 마라.** 그건 coverage 를 만족시킨 것이 아니라 회피한 것이다.
검증기는 용어집·수치 인덱스 절을 coverage 계산에서 빼므로 실제로 통과하지도 않는다.
원문 표를 해설 없이 통째 복사한 부록도 같은 이유로 금지한다.
...
```bash
python3 scripts/verify_explainer.py "<출력_ko_explained.md>" "<소스.md>" --production
```

소스를 **명시적으로 넘겨라** — 자동 탐색에 맡기면 폴더의 다른 `.md` 를 고르거나(오판),
staging 디렉터리에서는 아예 못 찾아 대조 검사가 통째로 생략된다.

`FAIL` 또는 `BLOCKING_REVIEW`(둘 다 rc 1) 이면 publish 하지 말고 해당 항목을 고친 뒤
**다시 돌린다 — rc 0 이 될 때까지 반복**한다. 검증기가 보는 항목:

| 무게 | 항목 |
|---|---|
| blocking | `number-coverage`(본문 수치) · `table-row-coverage`(표 행) · `math-coverage`(수식) · `image-refs` · `source-copy` · `register`(문체) · `adjacent-dup` · high-risk `ratio` · YAML 헤더 |
| advisory | 비유마커 · 비유 라벨 · 매체 과잉설명 · 웹 잡동사니 · 용어집 유무 · 저위험 `ratio` |

`ADVISORY_REVIEW`(rc 0) 는 자동 반려가 아니지만
**각 항목의 근거를 확인**하고(예: 해라체 문단이 정말 직접인용인지), 근거가 없으면 고친다.
정적 검증기가 `PASS` 여도 아래 항목은 여전히 사람/에이전트가 확인해야 한다 —
검증기는 **수치가 옳은 문맥에 놓였는지**, 비유의 적절성, 수식 해설의 정확성,
차트 전사 내용이 실물과 맞는지를 보지 못한다. 검증기는 "빠졌는가"만 보고 "맞는가"는 못 본다.
