...
`FAIL` 이면 publish 하지 말고 해당 항목을 고친 뒤 **다시 돌린다 — FAIL 이 사라질 때까지 반복**한다.
검증기가 보는 하드 항목: 소스 복사율(`source-copy`), 분량비(`ratio`), 이미지 참조 보존(`image-refs`),
문체 이중구조(`register`), 인접 중복(`adjacent-dup`), YAML 헤더. `REVIEW` 는 자동 반려가 아니지만
