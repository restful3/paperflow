...
12. **MCP `viewer_url` is auth-required**: `get_job_result` returns a convenience link. As of the by-id route it is `/viewer/by-id/{paperflow_source_id}` — a 302 redirect to the current `/viewer/{paper_name}`, so it survives folder rename/archive (resolves by durable source_id). Still gated by `get_current_user_page`: anonymous clicks redirect to `/login`. It is a logged-in operator's bookmark, NOT a public report link. The URL is opaque — do not parse `paper_name` out of it, and reaching the viewer requires following the redirect. If `MCP_PUBLIC_BASE_URL` points at `localhost`, the link is also host-local only — flag it that way in reports
13. **MCP `download_url` is bearer-only, not human-clickable**: zip endpoint requires `Authorization: Bearer <MCP_API_KEY>`. A raw `download_url` pasted into a report yields 401 on a normal browser click. Treat it as an agent-only retrieval URL; for long-lived reports, fetch the zip and upload it to your own artifact store (Paperclip attachment, etc.) — never embed the bearer token in the URL or document. `expires_at` reflects the MCP job-index TTL (~7 days); even before expiry, archive/delete of the paper folder will 410 the endpoint
14. **MCP `paper_name` is a convenience key, not a stable identifier**: smart folder rename happens before `status=complete`, so `get_job_result` returns the post-rename name, but later manual archive/restore/delete/rename or same-title reprocess can shift it. `paperflow_source_id` (= `expected_filename`, durable per job) and `location` (`outputs` | `archives`) are the resolution-safe pair for long-lived references. `safe_paper_dir_at_location(name, location)` is the helper that pins lookup to the recorded location. `get_job_result.viewer_url` is now built from `paperflow_source_id` via `/viewer/by-id/{source_id}` → 302 → current folder, so the human/agent link no longer breaks on rename; `resolve_paper_by_source_id(source_id)` is the disk-based resolver behind it (works beyond the job-index TTL)
