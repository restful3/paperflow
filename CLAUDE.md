...
12. **MCP `viewer_url` is auth-required**: `get_job_result` returns a convenience link to `/viewer/{quote(paper_name)}`, but `viewer_page` is gated by `get_current_user_page`. Anonymous clicks redirect to `/login`. It is a logged-in operator's bookmark, NOT a public report link. If `MCP_PUBLIC_BASE_URL` points at `localhost`, the link is also host-local only — flag it that way in reports
