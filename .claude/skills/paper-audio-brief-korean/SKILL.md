...
하한 미달은 과압축 실패로 보고 다시 늘린다. 짧은 출력이 "안전한 통과 경로"가 아니다 — 미달은 명백한 실패다. 구조 게이트(`##` 헤더 4+, 나열 금지, 마무리 한 줄)는 **tier·분량과 무관하게 항상 적용**한다.
...
- CRITICAL grep 0건 통과 여부 + 분량 밴드(tier 기준)·섹션 골격(헤더 4+) 통과 여부
