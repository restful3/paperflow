...
**대상이 2편 이상인 배치·재생성 작업은 Sonnet 워커 서브에이전트에 위임하는 것이 기본값이다** (2026-07-22 실측: 6/5\~6/6 결함 해설판 53편을 Sonnet 워커로 재생성, 오케스트레이터 감사에서 걸린 결함 3/53건·전부 경미, 비용 약 1/5):
...
- 워커는 산출물을 **스크래치패드 임시 경로에만** 쓴다 (논문 폴더는 읽기 전용, 기존 산출물 열람 금지 — 구버전 결함 학습 방지). 소스 분량 기준으로 워커당 여러 편을 배분해 스킬 읽기 비용을 상각한다.
- 오케스트레이터가 Quality Checks 게이트(자동: 배율·재진술 마커·퍼지 중복·해라체 스캔 + 표본 육안, 차트 전사는 이미지 파일 대조)를 실행하고, **통과분만** 폴더에 반영한다 — 구본은 `.bak-<timestamp>` 백업.
