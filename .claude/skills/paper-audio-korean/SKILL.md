...
- **기준 분량(낭독 대상 본문)** = 해설판 자수 − §제거 대상(참고문헌·감사의 글·용어집 표 등) − raw 데이터 부록(원문 보존형 — 규칙 4c에 따라 구조 요약으로만 서술되는 블록). 배너·메타 한 줄은 무시할 수준이므로 빼지 않는다. 자수는 `wc -m`으로 재되, rtk 훅 환경에서는 일반 `wc`가 0을 반환할 수 있으므로 **`rtk proxy wc -m`** 을 쓴다.
...
### 실행 모델 — Sonnet 워커 위임 (배치 기본값)

**대상이 2편 이상인 배치·재생성 작업은 Sonnet 워커 서브에이전트에 위임하는 것이 기본값이다** (2026-07-22 해설판 53편 재생성 실측: Sonnet 워커 결함 3/53건·전부 경미, 비용 약 1/5):

- 오케스트레이터는 Agent 도구(general-purpose)에 **`model: "sonnet"` 오버라이드**로 워커를 띄운다. 워커 프롬프트에는 이 SKILL.md 의 **경로를 주고 전체를 읽게 한다** (요약본을 넣지 않는다 — 규칙 누락의 원인).
- 워커는 산출물을 **스크래치패드 임시 경로에만** 쓴다 (논문 폴더는 읽기 전용). 소스 분량 기준으로 워커당 여러 편을 배분해 스킬 읽기 비용을 상각한다.
- 오케스트레이터가 Verification 게이트(자동 grep 전수 + 표본 육안, 그림 묘사는 이미지 파일 대조)를 실행하고, **통과분만** 폴더에 반영한다 — 구본은 `.bak-<timestamp>` 백업, sidecar(`_ko_audio.meta.json`) 기록 포함.
- 게이트 실패분·의심분만 오케스트레이터(상위 모델)가 직접 감사·수정한다. 알려진 Sonnet 워커 오류 패턴: 차트 축 단위 오독(예: €bn→"억" 10배 축소), 동아시아 인명 로마자 한글 음차 추정, 소스 풀쿼트 중복 이월 — 워커 프롬프트에 금지 조항으로 명시하고 감사 때 우선 확인한다.
- **단건 대화형 요청은 현재 모델이 직접 처리해도 된다.**
...
- [ ] **그림 묘사-실물 일치**: 임베딩된 그림 중 표본 2\~3개의 이미지 파일을 열어 묘사가 실제 내용(차트 유형·축·추세)과 일치하는지 확인했는가
